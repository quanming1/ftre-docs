...
| `AgentLoop._run_async()` | 构造 `agent_event`，包括 `user_input` echo 和 Agent 运行事件；`agent.run()` 正常 yield 出来的 Agent 事件中，`message_complete` / `reasoning_complete` / `tool_call` / `tool_result` / `user_message` / `usage_update` / `error` / `done` 会按 `_PERSISTENT_CLASSES` 白名单写入 DB（`message`、`reasoning`、`tool_call_streaming` 流式增量不持久化；`tool_cancel_requested` / `tool_cancelled` 无对应事件类，不产出也不持久化）。`_run_async()` 在主事件循环内直接 await 执行，不需要 `run_in_executor` 或 `asyncio.run()` |
