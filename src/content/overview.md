...
- **2026-08-08**：复验关键事实。Gateway 端口读取逻辑（`config.py:154-166` 的 `load_gateway_address`，从 `config.json` 的 `servers.gateway` 读 host/port，缺省 `127.0.0.1:48650`）与 `main.py:164-167` 调用一致；`ChannelManager` 中 `MIRROR_TO_WS_CHANNELS = {"cron"}`（`channel/manager.py:13`）和 `CronChannel` 在 `CronScheduler.__init__` 中通过 `channel_manager.register(CronChannel(bus))` 补注册（`tools/cron.py:124-125`）均与本文档描述一致；`AgentLoop` 仍为全局单例（`main.py:174-184` 创建并 start），并发模型 `_consume` → `create_task(_dispatch)` + per-session `asyncio.Lock` 串行（`agent/loop.py:275-326`）未变；Pipeline 三阶段（command → compact → run）和 `CompactManager.maybe_schedule_idle_compact` 仍在 `_run_async` 末尾及 `assistant_message_complete` 实时检查点调用，触发条件统一为 `precompact_threshold`（默认 0.5）。
